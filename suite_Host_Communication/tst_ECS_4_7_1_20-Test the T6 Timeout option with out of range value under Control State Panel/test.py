# -*- coding: utf-8 -*-
#Test the T6 Timeout option with out of range value under Control State Panels
import names
...
def main():
    ...
    T6_Timeout_valid_value=jsondata["T6_Timeout_valid_value"]
    T6_Timeout_invalid_value=jsondata["T6_Timeout_invalid_value"]
    ...
    snooze(2)
    mouseClick(waitForObject(names.ecs_Window), 40, 581, MouseButton.PrimaryButton)
    snooze(2)
    type(waitForObject(names.eCSUI_Window),"<Ctrl+A>")
    type(waitForObject(names.eCSUI_Window),"<Backspace>")
    snooze(2)
    type(waitForObject(names.eCSUI_Window),T6_Timeout_invalid_value)
    type(waitForObject(names.eCSUI_Window),"<Return>")
    snooze(2)
    clickButton(waitForObject(names.apply_Button))
    snooze(2)
    test.ocrTextPresent(str(T6_Timeout_valid_value))
    snooze(2)
