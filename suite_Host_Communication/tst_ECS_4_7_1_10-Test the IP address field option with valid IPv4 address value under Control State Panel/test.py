# -*- coding: utf-8 -*-
#Test the IP address field option with valid IPv4 address value under Control State Panel
import names
...
def main():
    ...
    snooze(2)
    type(waitForObject(names.eCSUI_Window), IPv4)
    snooze(2)
    type(waitForObject(names.eCSUI_Window), "<Return>")
    snooze(2)
    clickButton(waitForObject(names.apply_Button))
    snooze(2)
    test.compare(waitForObjectExists(names.o_Edit_9).text, IPv4)
    snooze(2)
